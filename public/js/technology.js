	...
	$.ajax({
		...
		success: function(data) {
			...
			if (data.error) {
				...
				$('#propiedadBtn').append(`<a class="btn" href="registroPropiedadIntelectual">Registrar Propiedad Intelectual</a>`);

				let auxStringTRL = ``;
				if (data.TRL.length === 0) {
					auxStringTRL += `<h6 class="red-text">Esta tecnología no ha sido evaluada.</h6>
                    <a href="trl/evaluarProyecto" class='btn'>Evaluar Ahora</a>`;
				} else {
					auxStringTRL += `
					<ul class="collapsible" id="div_evals">
						${data.TRL.map(evaluacion => {
							mensajeCero =
								evaluacion.nivel == 0
									? '<p class="red-text">Desafortunadamente con el avance actual del proyecto no se puede asignar un nivel TRL, puede encontrar más detalles en el reporte que hemos generado.</p>'
									: '';
							return `
						<li>
							...
							<div class="row">
								...
								<h6>Descargue un reporte con el análisis e interpretación de sus resultados</h6>
								${
									evaluacion.desbloqueado
										? `<a href="technology/genPDF/${evaluacion.id}" target="_blank" class='waves-effect waves-light btn'>Ver Reporte</a>`
										: `<a data-id="${evaluacion.id}" class='waves-effect waves-light btn lockedEval modal-trigger'  href="#modalLicencia">Descargar</a>`
								}</div>
								<div class="col s12">
								<h6>Resultados:</h6>
									<table class="tg">
				...
				if (data.canBeEvaluated) {	auxStringTRL += `<a href="trl/evaluarProyecto" class='waves-effect waves-light btn cta azul-tbr'><i class='material-icons right'>assignment_turned_in</i>Evaluar Ahora</a>`;
				} else {
					auxStringTRL += `<h6>Las tecnologias pueden ser evaluadas una vez cada 5 dias.</h6>`;
				}

				$('#evaluaciones').append(auxStringTRL);
